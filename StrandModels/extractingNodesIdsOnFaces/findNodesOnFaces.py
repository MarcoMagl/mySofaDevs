...
for e in entities:
    ...
    if len(physicalTags):
        ...
        for p in physicalTags:
            ...
            if n == "FaceFixed ":
                print "\n" + "Face found" + "\n"
                print "Exporting list of nodes"
                ids = np.array(nodeTags)
                np.savetxt("nodesFaceFixed.txt", ids, fmt="%d")
                FaceFixedFound = 1
                print("nodes fixed")
                print nodeTags
                print np.array(nodeCoords).reshape(-1,3)
            elif n == "FacePulled ":
                print "\n" + "Face found" + "\n"
                print "Exporting list of nodes"
                ids = np.array(nodeTags)
                np.savetxt("nodesFacePulled.txt", ids, fmt="%d")
                FacePulledFound = 1
                print nodeTags
                print np.array(nodeCoords).reshape(-1,3)
